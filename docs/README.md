...
`agentican-blocks` wraps five providers — **Anthropic**, **OpenAI**, **Google Gemini**, **AWS Bedrock**, and **OpenAI-compatible** endpoints (Groq, SambaNova, Together, Fireworks, and friends) — behind one `Client` / `ModelResponse` API, so switching providers is a one-line builder change instead of a code rewrite.
